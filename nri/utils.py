...
def my_softmax(input, axis=1):
    ...
    soft_max_1d = F.softmax(trans_input, dim=0)  #<-- updated (added explicit dim)
...
def load_data2(batch_size=1, subfolder='science', suffix='rc'):
    loc_train = np.load(f'nri/data/{subfolder}/loc_train{suffix}.npy')
    loc_valid = np.load(f'nri/data/{subfolder}/loc_valid{suffix}.npy')
    loc_test = np.load(f'nri/data/{subfolder}/loc_test{suffix}.npy')
    
    # [num_samples, num_timesteps, num_dims, num_atoms]
    num_atoms = loc_train.shape[3]

    # just do a complete graph
    edges_train = np.ones((loc_train.shape[0], num_atoms, num_atoms))
    edges_valid = np.ones((loc_valid.shape[0], num_atoms, num_atoms))
    edges_test  = np.ones((loc_test.shape[0], num_atoms, num_atoms))
...
def load_data(batch_size=1, subfolder='mini', suffix=''):
    loc_train = np.load(f'data/{subfolder}/loc_train{suffix}.npy')
    vel_train = np.load(f'data/{subfolder}/vel_train{suffix}.npy')
    edges_train = np.load(f'data/{subfolder}/edges_train{suffix}.npy')

    loc_valid = np.load(f'data/{subfolder}/loc_valid{suffix}.npy')
    vel_valid = np.load(f'data/{subfolder}/vel_valid{suffix}.npy')
    edges_valid = np.load(f'data/{subfolder}/edges_valid{suffix}.npy')

    loc_test = np.load(f'data/{subfolder}/loc_test{suffix}.npy')
    vel_test = np.load(f'data/{subfolder}/vel_test{suffix}.npy')
    edges_test = np.load(f'data/{subfolder}/edges_test{suffix}.npy')
...
def get_triu_indices(num_nodes):
    ...
    triu_indices = (ones.triu() - eye).nonzero(as_tuple=False).t()  #<-- updated
...
def get_offdiag_indices(num_nodes):
    ...
    ones = torch.ones(num_nodes, num_nodes)
    eye = torch.eye(num_nodes, num_nodes)
    offdiag_indices = (ones - eye).nonzero(as_tuple=False).t()
...
def get_triu_offdiag_indices(num_nodes):
    ...
    return torch.nonzero(triu_idx, as_tuple=False)
