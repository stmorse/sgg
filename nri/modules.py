...
from nri.utils import my_softmax, get_offdiag_indices, gumbel_softmax
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight.data)  # <-- updated
...
class CNNEncoder(nn.Module):
    ...
    def node2edge_temporal(self, inputs, rel_rec, rel_send):
        ...
        # rel_rec / send: [num_edges (atoms*(atoms-1)), atoms]
        x = inputs.view(inputs.size(0), inputs.size(1), -1)
        # now x has shape [num_sims, num_atoms, num_time * num_dims]

        # so last dimension has each position vector over time flattened
        # to one dimension.  x[0,0,:] is 0th sim, 0th atom, [t=0, t=1, t=2, ...]

        receivers = torch.matmul(rel_rec, x)
        # receivers now [sim, edges, time*dims]  (atoms subsumed in matmul)
        # receivers represents positions of receiving nodes for each edge
        # (note, constructed as complete graph)

        receivers = receivers.view(inputs.size(0) * receivers.size(1),
                                   inputs.size(2), inputs.size(3))
        # now [edges * sim, time, dims]
        # collapse across batches, separate back out time / dim
        
        receivers = receivers.transpose(2, 1)
        # now [edges * sims, dims, time]

        senders = torch.matmul(rel_send, x)
        senders = senders.view(inputs.size(0) * senders.size(1),
                               inputs.size(2), inputs.size(3))
        senders = senders.transpose(2, 1)
        # same for sending node

        # receivers and senders have shape:
        # [num_sims * num_edges, num_dims, num_timesteps]
        edges = torch.cat([senders, receivers], dim=1)
        # edges now [sims*edges, 2 * dims, time]
        return edges
